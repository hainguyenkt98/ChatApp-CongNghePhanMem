$(document).ready(function () {
    ...
    $(".btn.btn-secondary.un-block").off('click').on('click', function () {
        var userid = $(this).data('userid');
        $.ajax({
            url: "/BlockFriend/UnBlock",
            data: { userID: userid },
            dataType: "json",
            type: "post",
            success: function () {
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    s = s + '<li class="list-group-item list-group-item-light">'
                        ...
                        + '<button type="button" class="btn btn-secondary un-block " data-userid=' + result[i].Id + '>Bỏ chặn</button>'
                        + '</div>'
                        + '</div>'
                        + '</li >'
